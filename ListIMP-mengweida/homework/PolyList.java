...
*/
// 相减
    public static PolyList detel(PolyList p1,PolyList p2){
    ...
    //求导
    public static PolyList re(PolyList p5){
        PolyList resuit3=new PolyList();
        p5.current=p5.head.next;
        resuit3.insert(new PolyNode(p5.current.getA()*p5.current.getI(),p5.current.getI()-1));
        p5.current=p5.current.next;
        resuit3.insert(new PolyNode(p5.current.getA()*p5.current.getI(),p5.current.getI()-1));
        p5.current=p5.current.next;
        resuit3.insert(new PolyNode(p5.current.getA()*p5.current.getI(),p5.current.getI()-1));
        return resuit3;
    }
   public static PolyList x(PolyList P6){
        P6.current=P6.head.next;
        while (P6.current!=null){
            if (P6.current.getI()==1){
                return P6;
        }
